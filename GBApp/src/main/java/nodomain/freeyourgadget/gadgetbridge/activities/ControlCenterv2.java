...
public class ControlCenterv2 extends AppCompatActivity
    ...
    //Modded, added
    @Nullable //Bisa null karena gak berguna kalo gakda device polar.
    private PolarConnectionManager mPolarConnectionManager;


    protected int getLayoutId(){return R.layout.activity_controlcenterv2;}
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<GBDevice> deviceList = getDeviceList(); //deviceManager.getDevices(); //getDummyDevice(100);
        _LogFunKt.loge(this, "deviceList.size= " +deviceList.size());

        //Modded, cek apakah ada device Polar pada List.
        for(GBDevice device : deviceList){
            if(device.getType() == DeviceType.POLAR){
                mPolarConnectionManager= new PolarConnectionManager(this, device);
                try {
                    mPolarConnectionManager.connectToDevice(device.getAddress());
                } catch (PolarInvalidArgument e) {
                    e.printStackTrace();
                    _ActFragFunKt.toast(this, "Terjadi kesalahan saat menyambung ke perangkat polar: " +device.getName() +". \nError message= " +e.getMessage());
                }
            }
        }
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item) {
        ...
        if (itemId == R.id.action_settings) {
            ...
            return true;
        } else if (itemId == R.id.action_db_management) {
